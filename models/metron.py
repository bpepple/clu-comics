...
# ---------------------------------------------------------------------------
# Private data-conversion helpers (used by map_to_comicinfo)
# ---------------------------------------------------------------------------
...
def map_to_comicinfo(issue_data) -> Dict[str, Any]:
    ...
    # Mokkari model_dump() renames API "name" -> "story_titles"
...
# ---------------------------------------------------------------------------
# cvinfo file utilities (stateless, no API needed)
# ---------------------------------------------------------------------------
...
def parse_cvinfo_for_comicvine_id(cvinfo_path: str) -> Optional[int]:
    """Parse a cvinfo file for ComicVine series ID (from 4050-NNNNN URL)."""
    try:
        with open(cvinfo_path, 'r', encoding='utf-8') as f:
            content = f.read()
...
class MetronClient:
    """
    Metron API client wrapping the Mokkari library.

    Encapsulates session management, rate-limit handling, and all API
    interactions.  Unknown attribute access is delegated to the underlying
    Mokkari session so callers can still use raw session methods when needed.
    """

    def __init__(self, username: str, password: str):
        self._session = MokkariSession(
            username=username, passwd=password, user_agent=CLU_USER_AGENT
        )

    # ------------------------------------------------------------------
    # Delegation – lets callers use api.issue(), api.series(), etc.
    # ------------------------------------------------------------------

    def __getattr__(self, name: str):
        # Only called when the attribute isn't found on MetronClient itself.
        # Delegate to the underlying Mokkari session.
    ...
    def _handle_rate_limit(self, e: RateLimitError, attempt: int, context: str) -> bool:
        """Sleep and return whether to retry after a RateLimitError."""
    ...
    def _api_call(self, fn, context: str, default=None):
        """Call fn() with rate-limit retry and standard error handling."""
        for attempt in range(_RATE_LIMIT_MAX_RETRIES):
            try:
                return fn()
            except RateLimitError as e:
                if not self._handle_rate_limit(e, attempt, context):
                    return default
    ...
    def get_series_id_by_comicvine_id(self, cv_series_id: int) -> Optional[int]:
        """Look up Metron series ID using ComicVine series ID."""
        def _call():
            results = self._session.series_list({"cv_id": cv_series_id})
            if results:
        ...
        return self._api_call(_call, f"looking up CV ID {cv_series_id}")

    def get_issue_metadata(self, series_id: int, issue_number: str) -> Optional[Dict[str, Any]]:
        """
        Fetch issue metadata from Metron using the double-fetch pattern.

        First searches for the issue, then fetches full details by ID.
        """
        def _call():
            issues = self._session.issues_list({"series_id": series_id, "number": issue_number})
            if not issues:
            ...
            result = _to_dict(self._session.issue(metron_issue_id))
        ...
        return self._api_call(_call, f"fetching issue {issue_number} in series {series_id}")

    def get_releases(self, date_after: str, date_before: Optional[str] = None) -> List[Any]:
        ...
        return self._api_call(
            lambda: self._session.issues_list(params), "getting releases", default=[]
        ) or []

    def get_all_issues_for_series(self, series_id: int) -> List[Any]:
        """Retrieve all issues for a given series ID."""
        def _call():
            params = {"series_id": series_id}
            app_logger.info(f"Fetching issues for series_id: {series_id} with params: {params}")
            return self._session.issues_list(params)

        return self._api_call(_call, f"retrieving issues for series {series_id}", default=[]) or []

    def search_series_by_name(self, series_name: str, year: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        def _call():
            ...
            results = self._session.series_list({'name': series_name})
        ...
        return self._api_call(_call, f"searching for series '{series_name}'")
    ...
    def get_series_details(self, series_id: int) -> Optional[Dict[str, Any]]:
        ...
        def _call():
            series = self._session.series(series_id)
            if not series:
                return None
            publisher = getattr(series, 'publisher', None)
            publisher_name = getattr(publisher, 'name', None) if publisher else None
            ...
            app_logger.info(
                f"Metron series details: cv_id={result['cv_id']}, "
                f"publisher={result['publisher_name']}, year={result['year_began']}"
            )
            return result

        return self._api_call(_call, f"getting details for series {series_id}")

    def get_series_cv_id(self, series_id: int) -> Optional[int]:
    ...
    def scrobble_issue(self, metron_issue_id: int, date_read: str = "") -> bool:
        ...
        result = self._api_call(
            lambda: self._session.collection_scrobble(scrobble_data) is not None,
    ...
    def get_series_id(self, cvinfo_path: str) -> Optional[int]:
        """
        ...
        3. Updates cvinfo with the found series ID.
        """
        ...
        metron_id = self.get_series_id_by_comicvine_id(cv_id)
    ...
    def fetch_and_map_issue(self, cvinfo_path: str, issue_number: str) -> Optional[Dict[str, Any]]:
        """
        Fetch issue metadata and map to ComicInfo format.

        Combines get_series_id, get_issue_metadata, and map_to_comicinfo.
        Also saves publisher_name and start_year to cvinfo for future use.
        """
        series_id = self.get_series_id(cvinfo_path)
        if not series_id:
            app_logger.warning("Could not determine Metron series ID")
            return None

        issue_data = self.get_issue_metadata(series_id, issue_number)
        if not issue_data:
            return None
    ...
    def resolve_metron_issue_id(self, comic_path: str, issue_number: str = None) -> Optional[int]:
        """
        ...
        try:
            ...
            if not series_id and comic_info:
                ...
                if series_name:
                    try:
                        ...
                        year = None
                    search_result = self.search_series_by_name(series_name, year)
                    if search_result:
            ...
            # Step 3: Fetch all issues and match by number
            all_issues = self.get_all_issues_for_series(series_id)
            if not all_issues:
                return None
            ...
            for issue in all_issues:
                issue_num = (
                    ...
                    or (issue.get('number') if isinstance(issue, dict) else None)
                )
                if issue_num is not None:
                    ...
                    if candidate == target:
                        issue_id = (
                            ...
                            or (issue.get('id') if isinstance(issue, dict) else None)
                        )
                        if issue_id:
                            app_logger.info(
                                f"Resolved Metron issue ID {issue_id} for #{issue_number} "
...
# ---------------------------------------------------------------------------
# Module-level factory (backward-compatible entry point)
# ---------------------------------------------------------------------------

def get_api(username: str, password: str) -> Optional[MetronClient]:
    """
    Initialize and return a MetronClient.

    Returns None if credentials are missing or the session cannot be created.
    """
    if not username or not password:
        app_logger.warning("Metron credentials not configured")
        return None
    try:
        return MetronClient(username, password)
    except ApiError as e:
